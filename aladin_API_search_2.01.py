...
def fetch_toc(session, item_id):
    """상품 페이지에서 목차 스크래핑"""
    product_url = f"https://www.aladin.co.kr/shop/wproduct.aspx?ItemId={item_id}"

    # 1) 상품 페이지 방문 (쿠키 획득 + hd_ISBN 추출)
    resp = session.get(product_url, headers=HEADERS)
    if resp.status_code != 200:
        return ""

    match = re.search(r'class="hd_ISBN"\s+value="([^"]+)"', resp.text)
    if not match:
        return ""
    hd_isbn = match.group(1)

    # 2) Introduce 콘텐츠 요청 (목차 포함)
    time.sleep(0.3)
    contents_url = (
        f"https://www.aladin.co.kr/shop/product/getContents.aspx"
        f"?ISBN={hd_isbn}&name=Introduce&type=0&date={time.localtime().tm_hour}"
    )
    resp2 = session.get(
        contents_url,
        headers={**HEADERS, "Referer": product_url},
    )
    if resp2.status_code != 200 or len(resp2.text.strip()) == 0:
        return ""

    # 3) 목차 파싱 (div_TOC_All 우선, 없으면 div_TOC_Short)
    soup = BeautifulSoup(resp2.text, "html.parser")
    toc_div = soup.find("div", id="div_TOC_All") or soup.find("div", id="div_TOC_Short")
    if not toc_div:
        return ""

    return toc_div.get_text(separator="\n", strip=True)
...
def main():
    ...
    # 목차 수집
    print(f"\n목차 수집 중... (총 {len(all_items)}건)")
    session = requests.Session()
    for i, item in enumerate(all_items):
        item_id = item.get("itemId", "")
        if not item_id:
            continue
        toc = fetch_toc(session, item_id)
        item["toc"] = toc
        status = "O" if toc else "X"
        print(f"  [{i+1}/{len(all_items)}] {item['title'][:30]} ... 목차: {status}")
        time.sleep(0.5)
